...
namespace VSX.UniversalVehicleCombat.Radar
{
    ...
    public class HUDRadarWidget : MonoBehaviour
    {
        ...
        public virtual void UpdateRadarWidget(Trackable trackable)
        {
            ...
            if (hasFoot)
            {
                foot.localPosition = new Vector3(0f, -1 * transform.localPosition.y, 0f);
            }

            // Update the leg 
            if (hasLeg)
            {
                leg.localPosition = new Vector3(0f, -0.5f * transform.localPosition.y, 0f);
                leg.sizeDelta = new Vector2(leg.sizeDelta.x, Mathf.Abs(transform.localPosition.y));
            }
        }
    }
}
